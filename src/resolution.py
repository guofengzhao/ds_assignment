...
This script is an all-in-one resolution, which starts with helper functipons, 
then followed by the processing flow which starts from the comment line 
" ===== MAIN FLOW STARTS FROM HERE =====".

There are also two segmented scripts available which allow to run thhe 
solution in two phases -
* phase1_deduplicate.py: Cleanse and deduplicate the datasets and persist to 
                         intermediate csv files
*        phase2_link.py: Resolve the record linkage and generate the final
                         result
...
authors_similarity_threshold = 0.50
venue_similarity_threshold = 0.97
year_similarity_threshold = 0.99
