...
This is the second step of the phased processing.
...
def title_similarity_np(row1, row2, method = "difflib"):
    if method.lower() == "levenshtein":
        return 1- distance.nlevenshtein(str(row1[1]), str(row2[1]), method=1)
    if method.lower() == "sorensen":
        return 1- distance.sorensen(str(row1[1]), str(row2[1]))
    if method.lower() == "jaccard":
        return 1- distance.jaccard(str(row1[1]), str(row2[1]))
    return difflib.SequenceMatcher(None, str(row1[1]), str(row2[1])).quick_ratio()
...
#_, dblp_unique = deduplicate_np (dblp)

#save the cleaned dataset for phased processing
#dblp_unique.to_csv(fname_dblp_clean)
...
#_, scholar_unique = deduplicate_np (scholar)
...
#scholar_unique.to_csv(fname_scholar_clean)

# load prepared DBLP dataset
dblp_unique = pd.read_csv(fname_dblp_clean, quotechar='"', encoding='ansi', engine='python')
print ("Clean DBLP DataFrame demensions: {}".format(dblp_unique.shape))

# load prepared Scholar dataset
scholar_unique = pd.read_csv(fname_scholar_clean, quotechar='"', encoding='ansi', engine='python')
print ("Clean Scholar DataFrame demensions: {}".format(scholar_unique.shape))
...
dblp_scholar_links.to_csv(fname_result, index=False)
